...
public class ObjectDetector : MonoBehaviour
{
    ...
    private Transform hitTransform = null;  //마우스 피킹으로 선택한 오브젝트 임시 저장
    ...
    private void Update()
    {
        //마우스가 UI에 머물러 있을 때는 아래 코드가 실행되지 않도록 한다.
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if(Physics.Raycast(ray, out hit, Mathf.Infinity))
            {
                ...
                {
                    //Tower를 생성하는 SpawnTower 호출
                    towerSpawner.SpawnTower(hit.transform);
                }
                //타워를 선택하면 해당 타워의 정보를 출력하는 패널 On
        ...
        {
            //마우스를 눌렀을 때 선택한 오브젝트가 없거나 타워가 아니면
